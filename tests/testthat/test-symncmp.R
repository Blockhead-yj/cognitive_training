...
test_that("`symncmp` should deal with normal and abnormal data", {
  ...
  expect_named(result_symncmp, c("pc", "mrt", "dist_effect", "dist_effect_adj", "is_normal"))
  expect_equal(result_symncmp$pc, 0.93, tolerance = 0.01)
  expect_equal(result_symncmp$mrt, 568.373, tolerance = 0.001)
  expect_equal(result_symncmp$dist_effect, 5.80, tolerance = 0.01)
  expect_equal(result_symncmp$dist_effect_adj, 0.0102, tolerance = 1e-4)
  ...
  expect_named(result_abnormal, c("pc", "mrt", "dist_effect", "dist_effect_adj", "is_normal"))
  expect_true(is.na(result_abnormal$pc))
  expect_true(is.na(result_abnormal$mrt))
  expect_true(is.na(result_abnormal$dist_effect))
  expect_true(is.na(result_abnormal$dist_effect_adj))
