...
test_that("`srt` should deal with normal and abnormal data", {
  ...
  expect_silent(
    result_normal <- srt(
      jsonlite::read_json("data_srt/data_srt.json", simplifyVector = TRUE)
    )
  )
  ...
  # testing on data with too few valid responses
  expect_warning(
    result_abnormal <- srt(
      jsonlite::read_json("data_srt/data_srt_abnormal.json", simplifyVector = TRUE)
    ),
    "Valid response rate is less than 80%."
  )
